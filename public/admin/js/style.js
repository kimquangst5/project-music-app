...
const upploadAudio = document.querySelector(`[upload-audio]`);

if (upploadAudio) {
	// const audio = upploadAudio.querySelector(`audio`);
	const input = upploadAudio.querySelector(`input`);
	// let html = ''
	input.addEventListener('change', () => {
		const currentAudio = upploadAudio.querySelector('audio')
		if(currentAudio){
			upploadAudio.removeChild(currentAudio)
		}
		...
		if (file) {
			...
			const newAudio = new Audio(newLink);
			html = `<source class="p-[5px] rounded-[5px] border-[1px] text-[12px] bg-transparent border-text outline-none col-span-2" src="${newAudio.src}">`
			audio.innerHTML = html
		}
		upploadAudio.appendChild(audio)
		console.log("ok")
	});
}
...
// button-submit-form UPDATE CAP NHAT - cập nhật
